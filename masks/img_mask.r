...
dirD <- "/media/hkropp/Seagate Portable Drive"

img <- list.files(paste0(dirD, "/images"),pattern=".tif")
...
for(i in 1:length(img)){
  imgL[[i]] <- rast(paste0(dirD,"/images/",img[i]))
...
for(i in 1:length(water)){
  writeRaster(waterR[[i]], paste0("/media/hkropp/Seagate Portable Drive/training_test/masks/water/", waterName[i] ))
}
...
for(i in 1:length(shrubs)){
  writeRaster(shrubR[[i]], paste0("/media/hkropp/Seagate Portable Drive/training_test/masks/shrubs/", shrubName[i] ))
}
...
plot(polyL[[2]])
plot(polyR[[2]], add=TRUE, alpha=0.5)

for(i in 1:length(poly)){
  writeRaster(polyR[[i]], paste0("/media/hkropp/Seagate Portable Drive/training_test/masks/polygonal/", polyName[i] ))
}
...
for(i in 1:length(trees)){
  writeRaster(treesR[[i]], paste0("/media/hkropp/Seagate Portable Drive/training_test/masks/trees/", treesName[i] ))
}
